...
This section walks through a complete Deep Research workflow using `PydanTask`.
...
At a high level, a deep research run with `PydanTask` follows a simple loop:
...
3. **Capabilities execute tasks.** The supervisor schedules runnable tasks, and `PydanTask` executes them using the capability named in `TaskItem.capability` (typically `research_agent` for web research, `worker_agent` for general analysis, and `producer_agent` for synthesis).
...
6. **You get a final report.** `PydanTask.run()` returns a `PydanTaskRunResult` with:
...
2. Instantiate `PydanTask` for that objective.
...
async def main():
    ...
    # 2. Create a PydanTask instance configured for deep research
    da = PydanTask(
        objective=objective,
...
- `deep_research_run.json` – a structured dump of the entire `PydanTaskRunResult`.
...
- `PydanTask` initializes a shared `RuntimeState` (objective, empty plan, capability registry).

- `PydanTask` constructs and wires together:
  ...
  - A capability registry (`PydanTask._capability_registry`) that contains (by default):
...
- Execution is dependency-aware and parallelized:
  ...
  - `PydanTask` only runs tasks whose dependencies are all `COMPLETED`, and runs eligible tasks concurrently.
...
- Optionally adding custom capabilities via the `sub_agents` argument to `PydanTask`.
